...
module IoTaintTracking {
  ...
  private class MethodAndInterfaceTaintTracking extends TaintTracking::FunctionModel, Method {
    ...
    MethodAndInterfaceTaintTracking() {
      ...
      this.(Method).hasQualifiedName("io", "LimitedReader", "Read") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*PipeReader).Read(data []byte) (n int, err error)
      this.(Method).hasQualifiedName("io", "PipeReader", "Read") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*PipeWriter).Write(data []byte) (n int, err error)
      this.(Method).hasQualifiedName("io", "PipeWriter", "Write") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*SectionReader).Read(p []byte) (n int, err error)
      this.(Method).hasQualifiedName("io", "SectionReader", "Read") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*SectionReader).ReadAt(p []byte, off int64) (n int, err error)
      this.(Method).hasQualifiedName("io", "SectionReader", "ReadAt") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // Interfaces:
      ...
      this.implements("io", "ByteReader", "ReadByte") and
      ...
      this.implements("io", "RuneReader", "ReadRune") and
      (inp.isReceiver() and outp.isResult(0))
      or
      ...
      this.implements("io", "ByteWriter", "WriteByte") and
      (inp.isParameter(0) and outp.isReceiver())
      or
