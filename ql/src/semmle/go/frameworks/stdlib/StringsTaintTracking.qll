...
module StringsTaintTracking {
  ...
  private class MethodAndInterfaceTaintTracking extends TaintTracking::FunctionModel, Method {
    ...
    MethodAndInterfaceTaintTracking() {
      ...
      this.(Method).hasQualifiedName("strings", "Builder", "String") and
      (inp.isReceiver() and outp.isResult())
      or
      // signature: func (*Builder).Write(p []byte) (int, error)
      this.(Method).hasQualifiedName("strings", "Builder", "Write") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Builder).WriteByte(c byte) error
      this.(Method).hasQualifiedName("strings", "Builder", "WriteByte") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Builder).WriteRune(r rune) (int, error)
      this.(Method).hasQualifiedName("strings", "Builder", "WriteRune") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Builder).WriteString(s string) (int, error)
      this.(Method).hasQualifiedName("strings", "Builder", "WriteString") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Reader).Read(b []byte) (n int, err error)
      this.(Method).hasQualifiedName("strings", "Reader", "Read") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Reader).ReadAt(b []byte, off int64) (n int, err error)
      this.(Method).hasQualifiedName("strings", "Reader", "ReadAt") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Reader).ReadByte() (byte, error)
      this.(Method).hasQualifiedName("strings", "Reader", "ReadByte") and
      (inp.isReceiver() and outp.isResult(0))
      or
      // signature: func (*Reader).ReadRune() (ch rune, size int, err error)
      this.(Method).hasQualifiedName("strings", "Reader", "ReadRune") and
      (inp.isReceiver() and outp.isResult(0))
      or
      // signature: func (*Reader).Reset(s string)
      this.(Method).hasQualifiedName("strings", "Reader", "Reset") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Reader).WriteTo(w io.Writer) (n int64, err error)
      this.(Method).hasQualifiedName("strings", "Reader", "WriteTo") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Replacer).Replace(s string) string
      this.(Method).hasQualifiedName("strings", "Replacer", "Replace") and
      (inp.isParameter(0) and outp.isResult())
      or
