...
module EncodingGobTaintTracking {
  ...
  private class MethodAndInterfaceTaintTracking extends TaintTracking::FunctionModel, Method {
    ...
    MethodAndInterfaceTaintTracking() {
      ...
      this.(Method).hasQualifiedName("encoding/gob", "Decoder", "Decode") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Decoder).DecodeValue(v reflect.Value) error
      this.(Method).hasQualifiedName("encoding/gob", "Decoder", "DecodeValue") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Encoder).Encode(e interface{}) error
      this.(Method).hasQualifiedName("encoding/gob", "Encoder", "Encode") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Encoder).EncodeValue(value reflect.Value) error
      this.(Method).hasQualifiedName("encoding/gob", "Encoder", "EncodeValue") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // Interfaces:
