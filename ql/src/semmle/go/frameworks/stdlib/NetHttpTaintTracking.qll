...
module NetHttpTaintTracking {
  private class FunctionTaintTracking extends TaintTracking::FunctionModel {
    ...
    FunctionTaintTracking() {
      ...
      hasQualifiedName("net/http", "DetectContentType") and
      (inp.isParameter(0) and outp.isResult())
      or
  ...
  private class MethodAndInterfaceTaintTracking extends TaintTracking::FunctionModel, Method {
    ...
    MethodAndInterfaceTaintTracking() {
      ...
      this.(Method).hasQualifiedName("net/http", "Header", "Add") and
      (inp.isParameter(_) and outp.isReceiver())
      or
      // signature: func (Header).Clone() Header
      this.(Method).hasQualifiedName("net/http", "Header", "Clone") and
      (inp.isReceiver() and outp.isResult())
      or
      // signature: func (Header).Get(key string) string
      this.(Method).hasQualifiedName("net/http", "Header", "Get") and
      (inp.isReceiver() and outp.isResult())
      or
      // signature: func (Header).Set(key string, value string)
      this.(Method).hasQualifiedName("net/http", "Header", "Set") and
      (inp.isParameter(_) and outp.isReceiver())
      or
      // signature: func (Header).Values(key string) []string
      this.(Method).hasQualifiedName("net/http", "Header", "Values") and
      (inp.isReceiver() and outp.isResult())
      or
      // signature: func (Header).Write(w io.Writer) error
      this.(Method).hasQualifiedName("net/http", "Header", "Write") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (Header).WriteSubset(w io.Writer, exclude map[string]bool) error
      this.(Method).hasQualifiedName("net/http", "Header", "WriteSubset") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Request).AddCookie(c *Cookie)
      this.(Method).hasQualifiedName("net/http", "Request", "AddCookie") and
      (inp.isParameter(0) and outp.isReceiver())
      or
      // signature: func (*Request).Clone(ctx context.Context) *Request
      this.(Method).hasQualifiedName("net/http", "Request", "Clone") and
      (inp.isReceiver() and outp.isResult())
      or
      // signature: func (*Request).Write(w io.Writer) error
      this.(Method).hasQualifiedName("net/http", "Request", "Write") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Request).WriteProxy(w io.Writer) error
      this.(Method).hasQualifiedName("net/http", "Request", "WriteProxy") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // signature: func (*Response).Write(w io.Writer) error
      this.(Method).hasQualifiedName("net/http", "Response", "Write") and
      (inp.isReceiver() and outp.isParameter(0))
      or
      // Interfaces:
