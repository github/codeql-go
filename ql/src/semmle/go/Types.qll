...
class StructType extends @structtype, CompositeType {
  ...
  Field getFieldOfEmbedded(Field embeddedParent, string name, int depth, boolean isEmbedded) {
    ...
    exists(StructType embeddedType | embeddedType = embeddedParent.getType().getBaseType*().getUnderlyingType() |
      result = embeddedType.getOwnField(name, isEmbedded)
    )
  }

  private predicate hasFieldCand(
    string name, Field f, int depth, boolean isEmbedded
  ) {
  ...
  predicate hasField(string name, Type tp) {
    exists(int mindepth | mindepth = min(int depth | this.hasFieldCand(name, _, depth, _)) and
  ...
  override predicate hasMethod(string name, SignatureType tp) {
    exists(int mindepth | mindepth = min(int depth | this.hasMethodCand(name, _, depth)) and
